...
class CMADDPG:
    ...
    def __init__(self,obs_size:int,action_size:int,agent_num:int,gamma:float,
                 ...
                 constraints, eps_decay:int=1000,):
        """
        ...
        self.steps = 0
        self.agents = []
        self.dual_variable = torch.tensor([1 for c in constraints]).to(self.device)
        self.dual_optim = []
        for _ in self.dual_variable:
            self.dual_optim.append(torch.optim.Adam(self.dual_variable))
    ...
    def add_to_replay(self,obs_old:torch.tensor, action:torch.tensor, reward:torch.tensor,
                      obs_new:torch.tensor, cost:np.ndarray):
        """
        ...
        if reward.size()[0]==0:
            ...
            reward = reward + cost
        self.replay.add(TensorDict({"obs":obs_old, "act":act, "rew":reward, "nobs":obs_new, "cons":constraints}))
    ...
    def update(self):
        ...
        for i,agent in enumerate(self.agents):
            ...
            for j in range(self.batch_size):
                ...
                exp_ret = exp_ret - (self.constraints - self.dual_variable[i]*cons[j][i])
            exp_ret = torch.divide(exp_ret, self.batch_size)
            agent.policy_grad.zero_grad()
            self.dual_optim[i].zero_grad()
            exp_ret.backward()
            self.dual_optim[i].backward()
